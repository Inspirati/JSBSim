...
float FGSurface::GetBumpHeight()
{
  ...
  //values
  static const float maxGroundBumpAmplitude=(const float)0.4; // cast away warning - RobD
  float h = (float)(sin(x)+sin(7*x)+sin(8*x)+sin(13*x)); // cast away warning - RobD
  h += (float)(sin(2*y)+sin(5*y)+sin(9*y*x)+sin(17*y)); // cast away warning - RobD

  return (float)(h*(1/8.)*bumpiness*maxGroundBumpAmplitude); // cast away warning - RobD
}
